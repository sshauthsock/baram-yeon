...
type StatRankingItem struct {
	...
	Value     float64 `json:"value"` // interface{} -> float64 로 변경 (유지)
}
...
type SoulCalculationResult struct {
	...
	MaxLevelInfo MaxLevelInfo  `json:"maxLevelInfo"` // <-- [수정] 여기! `json:"json:"maxLevelInfo"` -> `json:"maxLevelInfo"`
}
...
// App holds the application-wide dependencies and cached data.
type App struct {
	...
	dataLoadMutex   sync.RWMutex // Protects access to cached data during loading
	isDataLoaded    bool         // Flag to indicate if initial data loading was successful
}
...
// loadAllCreatureData loads and merges all creature information from Firestore.
func (a *App) loadAllCreatureData(ctx context.Context) error {
	...
	for name, staticInfo := range staticInfoMap {
		...
		if levelStats, ok := statsMap[name]; ok {
			// Ensure stats are sorted by level for consistent output
...
// loadSoulExpTableData loads soul experience table data (hardcoded).
func (a *App) loadSoulExpTableData() {
	a.dataLoadMutex.Lock()
	defer a.dataLoadMutex.Unlock()
	log.Println("Loading soul exp table...")
	// 배열 길이가 26이 되도록 인덱스 25의 값을 추가 (임의 값, 실제 데이터에 따라 조정)
...
// loadChakDataFromFirestore loads chak data from Firestore and pre-calculates chak constants.
func (a *App) loadChakDataFromFirestore(ctx context.Context) error {
	...
	// Calculate chakConstants here, once on load
...
func main() {
	...
	errChan := make(chan error, 2) // Buffer for 2 potential errors (creature, chak)
	...
	// Soul exp table is in-memory, no I/O, no error to collect from a goroutine
	app.loadSoulExpTableData() // Can be called directly

	wg.Wait() // Wait for all data loading goroutines to finish
	close(errChan)

	app.dataLoadMutex.Lock() // Acquire lock before setting data loaded status
...
// checkDataReady is a helper to ensure data is loaded before processing requests.
...
// getAllData handles requests for all creature data.
...
func (a *App) calculateBond(c *gin.Context) {
	...
	a.dataLoadMutex.RLock() // RLock here to access a.creatureData
	defer a.dataLoadMutex.RUnlock()

	var creatureType string
	if len(req.Creatures) > 0 {
		// Find the type of the first creature to determine the category for set effects
...
// getRankingsHandler handles requests for bond or stat rankings.
func (a *App) getRankingsHandler(c *gin.Context) {
	...
	a.dataLoadMutex.RLock() // RLock for cached data
	defer a.dataLoadMutex.RUnlock()
...
// getSoulExpTable provides the cached soul experience table.
...
// calculateSoulHandler handles soul experience calculation requests.
...
// getChakData provides the cached chak data and constants.
func (a *App) getChakData(c *gin.Context) {
	...
	response := ChakDataResponse{
		...
		Costs:     a.chakCosts, // Costs are part of App struct
	}
...
// ================== 헬퍼 함수 (랭킹 계산 등) (App 외부에 유지, 필요한 데이터 인자로 받음) ==================
...
func calculateBondRankings(category string, allCreatureData []CreatureInfo) []BondRankingItem {
	...
	if len(allCategoryCreatures) < 6 {
		return []BondRankingItem{} // 6개 미만이면 조합 불가능
	}

	// 랭킹 조합 대상 환수를 선별 (최상위 등급, 25레벨 스탯 기준)
	// 25레벨 스탯 기준으로 정렬할 수 있도록 각 환수의 25레벨 스탯 정보 캐싱 (임시)
